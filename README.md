...
## 介绍（Introduce）
Recently, ChatGpt language models have attracted widespread attention in the industry due to their excellent human friendliness and surprising versatility. Many researchers are also deeply attracted by the reasons behind it. Research suggests that the success of such models is a combination of three factors: 1. The quantity of model parameters; 2. Instruction format dataset; 3. Enhanced learning methods. Firstly, the number of model parameters was first noticed after GPT2. Scholars have found that the number of model parameters must reach a certain volume in order to exceed the "critical point", also known as "intelligent emergence", and gain increasingly powerful capabilities, especially in the general field. On the contrary, if the number of model parameters is too small, it is difficult to remember and lead to "catastrophic forgetting". Currently, the success of LLAMA-7B, ChatGLM-6B, and ChatRWKV-7B reflects the fact that although previous GPT models have a larger volume, they have not repeatedly utilized their effectiveness. Secondly, the instruction format dataset is a newly created dataset that, from a cognitive perspective, is indeed very unique and even plays a central role compared to previous datasets. We will report on this in future topics. Thirdly, in terms of reinforcement learning methods, such large language models benefit from a new learning algorithm called "Human Assisted Reinforcement Learning" (RLHF), with Lora fine-tuning being the most commonly used and the LLAMA model being the most frequently used.<br>
...
我们也注意到，LLAMA和ChatGLM由于所使用的基本语料（尤其是语言）大相径庭，有人担忧这种以Alpaca lora的方式，会反而导致ChatGLM中文性能下降。其实，这并不是Lora权值交叉融合造成的问题，当然我们建议，在选用新数据集时，尽量选用自己需要的语言（如中文Alpaca数据集），但这并不会让适得其反.ChatRWKV的测试者报告，大模型具有惊人的语言结构学习和泛化能力，仅使用1%的中文语料+99%的英文语料，模型反而对中英文理解达到“互通”。不过，我们之所以建议数据集的语种，是因为Lora权值在组装进底座后，会存在“中英文混用”，这个问题很容易解决，你只要告诉它，“用中文回答我”，就可以了。
