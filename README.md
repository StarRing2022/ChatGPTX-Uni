...
即将发布：
1.多Lora文件嵌套融合，需要两个Lora的配置和形状一致，敬请关注！我们会基于peft，重新优化编写一套lora融合底层库，同时也增强理论性和实用性
2.由于属于“通用模型迁移”前瞻性研究（可能是这个前景，首创性思路），很多概念界定、技术使用仍没有清楚，本人时间精力、知识范围也有限，不免出现遗漏和需要补充之处，请谅解并帮助进一步论证，ReadMe正在重造，增加实验结果（我们是做类脑计算的，该项属于偶然发现，只能缓慢改进）
