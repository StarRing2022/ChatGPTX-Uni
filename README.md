...
## 1.介绍（Introduce）
Recently, the ChatGpt language model has attracted widespread attention in the industry due to its stunning human friendliness and surprising versatility. Many researchers are also deeply attracted by the reasons behind it. Research suggests that the success of such models is a combination of three factors: 1. the quantity of model parameters; 2. Instrcution format dataset; 3. Enhanced learning methods. Firstly, the number of model parameters was first noticed after GPT2. Scholars have found that the number of model parameters must reach a certain spatial volume in order to exceed the "critical point" threshold. This phenomenon may be called "intelligent emergence", which can gain strong momentum, especially in the general field. On the contrary, if the number of model parameters is too small, it can make learning difficult to remember and lead to "catastrophic forgetting". Currently, the success of LLAMA-7B, ChatGLM-6B, and ChatRWKV-7B reflects the fact that although previous GPT models have a large volume, they have not fully exploited their utility. Secondly, the instruction format dataset is a newly created dataset that, from the perspective of cognitive science, is indeed unique and even plays a central role compared to previous datasets. We will report on this in future topics. Thirdly, in terms of reinforcement learning methods, such large language models do benefit from a new learning algorithm called "Human Assisted Reinforcement Learning" (RLHF), with Lora fine-tuning being the most commonly used and the most frequently used on the LLAMA model.<br>
...
## 2.方法（Method）
# 2.1 Lora融合（Lora-Fusion）
This study proposes a Lora fine-tuning method suitable for ChatGPT class large language models. The main contribution is to attempt to use Lora weights as a starting point to broaden the communication channels of various different language models, in order to achieve complementary and complementary functions. We were mainly inspired by the Peft library and also benefited from its excellent fine-tuning adaptation ability to different models. Taking ChatGLM-6B as an example, the official fine-tuning method is P-Tuning, and some researchers have obtained a low-priced Lora fine-tuning method based on the transformer's original trainer. The Peft library supports all the fine-tuning methods mentioned above for ChatGLM. Our effort is to fine tune the LLAMA-7B model on the Chinese Alpaca dataset using Lora to obtain a Lora weight file, which is then embedded into the ChatGLM pre trained model. The verification shows that this is a simple and efficient technical solution that enhances the base of small models with large model weights. Similarly, regardless of whether the two models belong to the same class or not, it is also a considerable model interaction bridge. Overall, we have announced this method, known as "Lora weight cross fusion", which not only has strong universality but also obvious effects. More importantly, the cost of computing resources is very low.<br>
...
数据集：<br>
英文原版Alpaca（引用于 https://github.com/THUDM/ChatGLM-6B/issues/599） <br>
中文机器翻译Alpaca(引用于 https://github.com/LC1332/Chinese-alpaca-lora) <br>
...
## 6.结论与后续课题(Conclusion and Future Work)
This study preliminarily attempts to use the peft fine-tuning library to construct association regions for ChatGpt class large language models of different types, volumes, and configurations through the Lora weight fusion method. Our experiment selected ChatGlm-6B as the base small model, while the slightly larger LLAMA-7B was translated into Chinese into the Alpaca dataset, and the Lora weight obtained by the Lora method was used as the component. Finally, the Lora weight was integrated into ChatGlm-6B, and preliminary results showed that this technical approach achieved satisfactory results. Subsequently, we also found that the Lora weight cross fusion strategy has excellent transfer ability, and its robustness is also excellent due to its reliance on a powerful and stable Peft library. In the future, we will continue to obtain Lora weights on different datasets, not limited to the paradigm of "LLAMA Lora+ChatGLM pre training model". Meanwhile, as mentioned earlier, we will further publish research on instruction datasets and, if possible, add brain like computing components, hoping to promote the "intelligent emergence" of small language models (SLM).<br>
